...
describe('PrismaMemoryStore integration', () => {
  it('atomically creates all five records and preserves exact evidence', async () => {
    ...
    const persisted = await prisma.run(async (client) => ({
      ...
      currentFact: await client.currentFact.findUniqueOrThrow({ where: { factId: record.fact.id } }),
  ...
  it('rolls back every record when the final projection insert fails', async () => {
    await prisma.run(async (client) => {
      await client.$executeRawUnsafe(`CREATE FUNCTION slice01_fail_current_fact_insert() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'synthetic current_fact failure'; END; $$ LANGUAGE plpgsql`);
      await client.$executeRawUnsafe(`CREATE TRIGGER slice01_fail_current_fact BEFORE INSERT ON current_facts FOR EACH ROW EXECUTE FUNCTION slice01_fail_current_fact_insert()`);
    });
  ...
  it('matches literal substrings case-insensitively', async () => {
    ...
    expect(hit).toMatchObject({ memoryId: record.memory.id, evidenceId: record.evidence.id, factId: record.fact.id, content: record.fact.content });
  });
  ...
  it('orders matches by newest recordedAt and then factId ascending', async () => {
    const older = buildRecord({ text: 'alvo antigo', recordedAt: new Date('2026-08-16T08:00:00.000Z') });
    const tieHigh = buildRecord({ text: 'alvo empate alto', recordedAt: new Date('2026-08-16T10:00:00.000Z'), factId: '0198c000-0000-7000-8000-000000000002' });
    const tieLow = buildRecord({ text: 'alvo empate baixo', recordedAt: new Date('2026-08-16T10:00:00.000Z'), factId: '0198c000-0000-7000-8000-000000000001' });
  ...
  it('atomically corrects current text while preserving original rows and recordedAt', async () => {
    ...
    await expect(store.findLiteral('corrigido')).resolves.toMatchObject({ memoryId: original.memory.id, content: 'Texto corrigido.', recordedAt: original.memory.recordedAt });
  ...
  it('serializes concurrent corrections so only one succeeds from the same expected fact', async () => {
    ...
    const [first, second] = await Promise.all([
      store.correct({ memoryId: original.memory.id, expectedCurrentFactId: original.fact.id, text: 'Versão A.', correctedAt, ids: correctionIds() }),
      store.correct({ memoryId: original.memory.id, expectedCurrentFactId: original.fact.id, text: 'Versão B.', correctedAt, ids: correctionIds() }),
    ]);
  ...
  it('rolls back the entire correction when current projection update fails', async () => {
    ...
    await prisma.run(async (client) => {
      ...
      await client.$executeRawUnsafe(`CREATE TRIGGER slice02_fail_current_fact_update BEFORE UPDATE ON current_facts FOR EACH ROW EXECUTE FUNCTION slice02_fail_current_fact_update()`);
    });
    await expect(store.correct({ memoryId: original.memory.id, expectedCurrentFactId: original.fact.id, text: 'Depois da falha.', correctedAt: new Date('2026-08-17T05:02:00.000Z'), ids: correctionIds() })).rejects.toThrow('synthetic correction projection failure');
    expect(await counts()).toEqual([1, 1, 1, 1, 1]);
    await expect(store.findLiteral('Antes da falha')).resolves.toMatchObject({ factId: original.fact.id });
  });
  ...
  it('returns one-version history for an original and semantic order after repeated corrections', async () => {
    ...
    expect(initialHistory?.versions).toEqual([
      expect.objectContaining({ factId: original.fact.id, evidenceId: original.evidence.id, content: 'Versão original.', eventId: original.event.id, reason: null, supersedesFactId: null, isOriginal: true, isCurrent: true }),
    ]);
    const first = await store.correct({ memoryId: original.memory.id, expectedCurrentFactId: original.fact.id, text: 'Versão um.', reason: 'primeira', correctedAt: new Date('2026-08-17T05:03:00.000Z'), ids: correctionIds() });
    expect(first.status).toBe('CORRECTED');
    if (first.status !== 'CORRECTED') throw new Error('expected first correction');
    const second = await store.correct({ memoryId: original.memory.id, expectedCurrentFactId: first.record.fact.id, text: 'Versão dois.', reason: 'segunda', correctedAt: new Date('2026-08-17T05:04:00.000Z'), ids: correctionIds() });
    ...
    expect(history?.versions.map((version) => version.factId)).toEqual([original.fact.id, first.record.fact.id, second.record.fact.id]);
    expect(history?.versions.map((version) => version.reason)).toEqual([
      null,
      'primeira',
      'segunda',
    ]);
